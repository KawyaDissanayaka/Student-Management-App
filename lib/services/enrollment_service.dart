...
class EnrollmentService {
  ...
  // Enroll new Student to a Subject
  Future<void> enrollStudent(EnrollmentModel enrollment) async {
    try {
      // Check for duplicate enrollment (same student, subject, semester, academic year)
      final duplicateQuery = await _enrollmentsRef
          .where('studentId', isEqualTo: enrollment.studentId)
          .where('subjectCode', isEqualTo: enrollment.subjectCode)
      ...
      if (duplicateQuery.docs.isNotEmpty) {
        throw Exception(
            'Student is already enrolled in this subject for ${enrollment.semester} (${enrollment.academicYear}).');
      }
